...
| H1.34 | Complete GDPR export/erasure, tamper-evident audit and retention workflows | partial | Garage export/erasure and permanent-customer deletion integration are complete. Audit rows now use a deployment-secret-salted SHA-256 hash chain serialized by a SQL application lock; legacy rows chain on first append, integrity verification detects content/link tampering, and daily seven-year retention pruning preserves the last removed hash as a chain anchor. Cross-domain account/order subject export with legal retention holds remains |
