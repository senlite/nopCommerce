...
| H1.6a | Support the other top-10 passenger brands besides BMW (WMI, hierarchy, documented VDS only) | in-progress | **Brands (BMW already shipped in H1.4/H1.6):** Toyota, Volkswagen, Honda, Hyundai, Ford, Mercedes-Benz, Nissan, Kia, Chevrolet. Domain stays brand-agnostic (`FR-204`, `INV-013`); each make is catalog JSON plus the existing `TP_CE_*` tables. **Slice 1:** NHTSA vPIC WMI allow-lists, `CatalogVinDecoder` fail-closed `vin.decode_failed`, minimal trees, documented VDS only. **Slice 2:** deeper flagship trees, EN/AR configuration aliases, additional WMIs, 73 VDS prefixes. **Slice 3:** more NHTSA flagships (Golf, Odyssey, GLE, Sienna, Ranger/Bronco, Kona, Colorado/Traverse, etc.), 107 documented non-BMW VDS prefixes, and a check-digit-valid golden VIN corpus through `VinDecodeApplicationService`. **Slice 4 (this change):** VIN picker labels include market name and production years so ECE vs GCC candidates are unique. **Later slices:** still more provenanced VDS — never fabricated OEM-complete maps |
...
- **H1.6a top-10 brands besides BMW** is the active vehicle/VIN workstream (plugin 0.99.0 slice 4:
  VIN picker labels include market and years). Later slices still add provenanced VDS the same
  way — add JSON, never manufacturer literals in Domain, never fabricate VDS maps.
