...
| H2.3 | Parse natural language into structured vehicle/part intent | partial | `SearchIntentVehicleResolver` resolves make/model/year to configuration; NL queries combine part keywords + semantic with fitment filter; versioned prompt store + VIN redaction |
...
| H2.6 | Persist AI descriptions, specifications, translations and SEO as review candidates | partial | Import publish rebinds pending candidates from row number to `publishedProductId`; review board applies approved candidates via `NopAiContentApplicator` |
| H2.7 | Enforce the controlled automotive translation glossary | partial | External `automotive-glossary.json` resource; glossary validation blocks approval when `qualityScore < 1` on translation candidates |
| H2.8 | Complete reviewable AI fitment candidate workflow | partial | Fitment inference parses JSON verdict from versioned prompt; reject no longer re-enqueues to review queue |
