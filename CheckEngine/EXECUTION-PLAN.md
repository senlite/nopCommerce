...
| H1.7 | Complete multi-candidate VIN disambiguation and privacy verification | partial | Garage now fails closed with HTTP 409 + ranked candidates instead of silently selecting candidate 0; search already refuses ambiguity. Customer picker, authoritative VIN corpus and explicit redaction/log audit remain |
...
| H1.36 | Implement online/offline licence activation and expiry read-only behavior | partial | `SqlLicenceStateStore` persists heartbeat state in `TP_CE_LicenceState`; production activation authority and vendor signing key remain external |
