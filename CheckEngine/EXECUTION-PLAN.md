...
| H1.6a | Support the other top-10 passenger brands besides BMW (WMI, hierarchy, documented VDS only) | in-progress | **Brands (BMW already shipped in H1.4/H1.6):** Toyota, Volkswagen, Honda, Hyundai, Ford, Mercedes-Benz, Nissan, Kia, Chevrolet. Domain stays brand-agnostic (`FR-204`, `INV-013`); each make is catalog JSON plus the existing `TP_CE_*` tables. **Slice 1:** NHTSA vPIC WMI allow-lists, `CatalogVinDecoder` fail-closed `vin.decode_failed`, minimal trees, documented VDS only. **Slice 2 (this change):** deeper flagship trees (EN/AR configuration aliases for VIN disambiguation labels), additional NHTSA `GetWMIsForManufacturer` WMIs, and more `DecodeVinValues` VDS prefixes (73 non-BMW patterns). Same-VDS different-generation rows (e.g. Ford Explorer `5K8`) disambiguate on ISO year windows. **Later slices:** still more provenanced VDS — never fabricated OEM-complete maps |
...
- **H1.6a top-10 brands besides BMW** is the active vehicle/VIN workstream (plugin 0.97.0 slice 2:
  deeper flagship trees, configuration aliases, more NHTSA WMIs/VDS). Expand catalogs the same
  way — add JSON, never manufacturer literals in Domain.
