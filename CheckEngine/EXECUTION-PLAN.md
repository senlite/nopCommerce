...
| H1.7 | Complete multi-candidate VIN disambiguation and privacy verification | partial | Storefront picker modal handles HTTP 409 and guest `/check-engine/vin/decode` disambiguation with configuration labels; authoritative VIN corpus and explicit redaction/log audit remain |
...
| H1.21 | Prove a 10,000-row batch through review/publication within the documented SLA | partial | Deterministic 10,000-row corpus runs all twelve stages and publishes every above-threshold row through `RecordingImportProductPublisher` + durable SQL-shaped repository at ≥50 rows/s. `NopImportProductPublisher` contract tests and DI wiring to `SqlImportPipelineRepository` are in place; live SQL Server + nopCommerce catalog publication rehearsal on a disposable clone remains before the five-working-day acceptance can be closed |
...
| H1.24 | Complete licensed supplier-image sourcing and batch replacement workflow | partial | Batch replacement by SKU list is now implemented (`ImageAdmin/ReplaceBatch`): each item resolves SKU→product, replaces the primary image in place (preserving product/gallery links), is quarantine-checked, and reports a per-item outcome (replaced/quarantined/not-found/failed) without one bad row blocking the rest; the batch is audited. Automated licensed supplier-image *sourcing* remains blocked on a licensed image provider/credentials |
...
| H1.28 | Meet Core Web Vitals on throttled mid-range mobile hardware | partial | `DefaultSeoPerformanceBudgetService` now exposes NFR-054 thresholds and `CheckEngine/scripts/run-cwv-gate.sh` provides an operator Lighthouse rehearsal gate; measured throttled-device evidence remains |
...
| ID | Task | Status | Gap |
|---|---|---|---|
| H1.31 | Synchronize products, inventory, customers, orders, invoices, returns and shipments bidirectionally | partial | Every outbound entity flow is now event-driven and durable: product update emits product+inventory jobs, customer register/update emits customer jobs, order placement/status emits order jobs, payment emits invoice, refund emits return, and shipment create/send emits shipment. Versioned idempotency keys preserve distinct updates and all consumers fail soft. Inbound ERP→nopCommerce mutation remains blocked on the external ERPNext doctype/webhook contract and field mappings |
...
| H1.36 | Implement online/offline licence activation and expiry read-only behavior | partial | `SqlLicenceStateStore` persists heartbeat state; `CheckEngineLicenceWriteFilter` blocks admin mutations after the 30-day grace period while storefront and diagnostics/uninstall remain reachable; production activation authority and vendor signing key remain external |
