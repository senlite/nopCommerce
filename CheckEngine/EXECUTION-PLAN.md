...
| H3.7 | Attributed, reviewable and revocable vendor fitment contributions | done | `TP_CE_FitmentClaim.VendorId` (nullable, null = operator); `VendorFitmentContributionService` submit/list/revoke with isolation + audit; vendor `RevokeFitmentProposal` + admin `Revoke`; legacy `SourceReference` fallback; plugin 0.60.0 |
