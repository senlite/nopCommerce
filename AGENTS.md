...
- One-time setup (OAuth on your PC, base64-encode `rclone.conf`, add secret `RCLONE_CONFIG_B64` and `RCLONE_CONFIG_B64_2` if over 4096 chars) is documented in `docs/cloud-agent-onedrive-setup.md`. A newly added secret is only injected into **new** agent runs.
