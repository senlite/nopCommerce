...
public sealed class AzureOpenAiEmbeddingPort : IAiEmbeddingPort
{
    private readonly CheckEngineAiOptions _options;
    private readonly HttpClient _httpClient;

    public AzureOpenAiEmbeddingPort(CheckEngineAiOptions? options = null, HttpClient? httpClient = null)
    {
    ...
    public async Task<AiEmbeddingResult> EmbedAsync(AiEmbeddingRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                {
                    Success = false,
                    ProviderName = "azure-openai-embeddings",
            ...
            return new AiEmbeddingResult
            {
                ...
                Vector = vector,
                ProviderName = "azure-openai-embeddings",
                ModelHash = VectorMath.HashModel(deployment, vector.Length),
    ...
    {
        if (!string.IsNullOrWhiteSpace(_options.AzureEmbeddingDeploymentName))
            return _options.AzureEmbeddingDeploymentName;

        return _options.AzureDeploymentName;
    }
